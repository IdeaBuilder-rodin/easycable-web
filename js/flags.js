...
WE.flags = {
  ...
  LAUNCH: true,
  /* 라이브러리 부품 자동 수집(js/libsync.js) — 배포로 끄는 스위치. 서버 쪽 스위치(LIBSYNC_MODE secret)는
     재배포 없이 끈다. 둘 중 하나만 꺼져도 안 보낸다. (2026-09-14) */
  LIBSYNC: true,

  /* ── 무료 한도를 적용할 것인가 (배선 30 · 부품 10 · 페이지 1 · 표 내보내기) ──────
     지금 값: **false — 아무에게도 한도가 걸리지 않는다.** (2026-09-10 확정, 아래 참고)

     ⚠ 아래 문단은 2026-09-07 에 값이 true 였을 때 적은 것이다. 그때의 판단 근거를
       남겨 두되, **지금 값은 false 다.** 다시 켤 때 이 설명을 먼저 읽는다.

     ⚠⚠ 실채널 전환일에 이 값을 반드시 다시 본다.
...
/* 미리보기 띠는 2026-09-01 출시 준비로 없앴다 (에디터 = 지금의 app.html 에서 요소째 삭제).
